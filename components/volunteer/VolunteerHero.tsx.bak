...
const HeroSection = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const overlayElement = heroRef.current.querySelector(".overlay") as HTMLElement;
      const contentElement = heroRef.current.querySelector(".content") as HTMLElement;
      
      if (overlayElement) {
        overlayElement.style.opacity = (0.5 + scrollPosition * 0.001).toString();
      }
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0">
        <Waves
          lineColor={
            theme === "dark" ? "rgba(255, 255, 255, 0.15)" : "rgba(255, 255, 255, 0.2)"
          }
      ...
      <div
        className="overlay absolute inset-0 bg-gradient-to-b from-[var(--accent-black)] from-10% via-[var(--accent-black)]/70 to-[var(--accent-black)]/90 opacity-50"
      ></div>
      ...
      <div className="content relative z-10 container mx-auto h-full flex flex-col justify-center px-4 sm:px-6">
        ...
        >
          ...
          >
            Black history is at risk of being erased. Join our movement to safeguard our past and ensure future generations know their roots.
          </motion.p>
          ...
          >
            ...
            <Button href="/volunteer" variant="outline" size="lg" className="border-white text-white hover:bg-white hover:text-[var(--accent-black)]">
              Get Involved
            </Button>
          </motion.div>
        </motion.div>
      ...
      >
        <span className="font-helvetica text-white text-sm mb-2">Scroll to explore</span>
        <motion.div
          animate={{
            y: [0, 10, 0],
          }}
